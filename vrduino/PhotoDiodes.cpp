...
bool PhotoDiodes::updateClockTicks() {
  ...
  for (int i = 0 ; i < 4 ; i++) {
    ...
    int value = s->poll();
    if(debug) Serial.printf("Value returned from sensor %d polling: %d: \n", i, value);
    ...
    if(newTimingVal){
      ...
      if(lighthouseToWatch_ == lighthousePolled_){ //our polled lighthouse matches the desired lighthouse to update.
        ...
        if(debug) {
          ...
          Serial.printf("values used for comp: sensor: %d, lighthouse: %d, %d, %d \n", i, lighthousePolled_, clockTicks_[2 * i], clockTicks_[2 * i + 1]);
          Serial.println("======================================================================================================");
        }
      }
    }
  ...
  for (int i = 0 ; i < 4 ; i++) {
    //poll all sensors to clear their buffers for the next timing measurements.
    LighthouseSensor * const s = &sensors_[i];
    int value = s->poll();
  }
...
{
  for (int i = 0; i < 4; i++) {
    float delta_tx = float(clockTicks_[2 * i]) / CLOCKS_PER_SECOND;
    float x_angle = -delta_tx / ((1. / 60) / 360.) + 360 / 4;
    projection2D_[2 * i] = tan(x_angle / 360 * 2 * PI);

    float delta_ty = float(clockTicks_[2 * i + 1]) / CLOCKS_PER_SECOND;
    float y_angle = delta_ty / ((1. / 60) / 360.) - 360 / 4;
    projection2D_[2 * i + 1] = tan(y_angle / 360 * 2 * PI);
  }
}
